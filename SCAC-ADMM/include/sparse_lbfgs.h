...
class SparseLBFGSSolver : public Learner {
  public:
    ...
    void FetchLocalUpdateIdx(dmlc::RowBlockIter<unsigned> *dtrain) {
      ...
      related_index_.resize(num_fea, 0);
      related_num_.resize(num_fea, 0);
      int block_size = num_fea/num_procs;
      int send_count;
      float  _0_10 = 0.0;
      float _10_100=0.0;
      float _100_1000=0.0;
      float _1000_10000=0.0;
      float _10000=0.0;
     ...
     MPI_Allreduce(&local_idx_freq[0], &idx_freq[0], num_fea, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
      for(int idx = 0;idx < local_idx_freq.size();idx++) {
	    ...
	    related_index_[idx] = 1;
	}
	if(idx_freq[idx]>=1 && idx_freq[idx]<=3)
	    _0_10++;
	else if(idx_freq[idx]>3&&idx_freq[idx]<=10)
	    _10_100++;
	else if(idx_freq[idx]>10&&idx_freq[idx]<100)
	    _100_1000++;
	else if(idx_freq[idx]>100&&idx_freq[idx]<1000)
	    _1000_10000++;
	else if(idx_freq[idx]>1000)
	    _10000++;
      }
      float new_1=0;
      float new_2=0;
      ...
      float new_4=0;
      float new_5=0;
      //MPI_Allreduce(&_0_10,&new_1, 1, MPI_FLOAT,MPI_SUM,MPI_COMM_WORLD);
      if(rank == 0){
      printf("1-5:%f\n",_0_10/num_fea);
      printf("5_10:%f\n", _10_100/num_fea);
      printf("10_100:%f\n", _100_1000/num_fea);
	...
	if(related_index_[block_size+j] == 1)
	    nnz ++;
      }

      if(rank == num_procs - 2)
	  need_send_idx = new int[(num_fea-(num_procs-1)*block_size)*num_procs];
      else
	  need_send_idx = new int[block_size*num_procs];
      for(int i = 0; i < num_procs; i ++){
	  if(i == num_procs - 2){
	      send_count = num_fea - (num_procs -1)* block_size;
	  } else {
	      send_count = block_size;
	  }
	MPI_Gather(&related_index_[0]+((i+1)%num_procs)*block_size, send_count, MPI_INT, need_send_idx, send_count, MPI_INT, i, MPI_COMM_WORLD);
      }
      MPI_Allreduce(&related_index_[0], &related_num_[0], num_fea, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
      int max_related_num = 0;
      for(uint32_t i = 0; i < num_fea; i ++)
	 // if(related_num_[i] > max_related_num)
	      max_related_num += related_index_[i];
      if(rank == 0){
	printf("fiter_threshold:");
	printf("%d\n", filter_threshold);
	printf("need update idx cnt=%d\n",need_update_idx.size());
      }
      if(rank == 0)
        LOG(INFO) << "filter threshold " << filter_threshold << ", need update idx cnt " << need_update_idx.size() << ", max_related_num" << max_related_num;
    }
