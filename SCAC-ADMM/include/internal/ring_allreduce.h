...
void RingAllreduce(T *buffer, int count, int id, int num_procs, MPI_Comm comm) {
    //worker_list包含了参与此次Allreduce的所有进程id
    int worker_number = num_procs;
    ...
    my_index = id;
    left = (id-1+worker_number)%worker_number;
    right = (id+1)%worker_number;
...
template<class O, class T>
void SparseRingAllreduce(T *buffer, int count, int id, int num_procs,int *need_send_index,int *traffic, int record_block_index,int type, int &sparse_in_reduce, int &sparse_in_gather, MPI_Comm comm) {
    int worker_number = num_procs;
    if (worker_number == 1) return;
    /*if (count < 512 * worker_number) {
        SimpleAllreduce<O>(buffer, count, id, worker_list, comm);
    ...
    blocks[worker_number - 1].second = (count - (worker_number - 1) * block_size);
    int left = -1, right = -1, my_index = id;
    /*for (int i = 0; i < worker_number; ++i) {
        if (worker_list[i] == id) {
            my_index = i;
            left = worker_list[(i - 1 + worker_number) % worker_number];
    ...
    for (int i = 0; i < worker_number - 1; ++i) {
		...
		sparse_in_reduce ++;
                ...
                MPI_Isend(value_buffer, nnz * (isize + vsize), MPI_CHAR, right,
                          MessageType::kScatterReduce, comm, &requests[0]);
		//if(send_block_index == record_block_index)
		traffic[send_block_index*2*(num_procs-1)+i]= nnz * (isize + vsize);
            } else {
                MPI_Isend(base, block_size * vsize, MPI_CHAR, right, MessageType::kScatterReduce, comm,
                          &requests[0]);
		//if(send_block_index == record_block_index)
		traffic[send_block_index*2*(num_procs-1)+i] = block_size * vsize;
            }
        } else {
            MPI_Isend(base, block_size * vsize, MPI_CHAR, right, MessageType::kScatterReduce, comm,
                      &requests[0]);
    ...
    for (int i = 0; i < worker_number - 1; ++i) {
		...
		sparse_in_gather++;
                ...
                MPI_Isend(value_buffer, nnz * (isize + vsize), MPI_CHAR, right,
                          MessageType::kAllGather, comm, &requests[0]);
		//if(send_block_index == record_block_index)
		traffic[send_block_index*2*(num_procs-1)+worker_number-1+i] = nnz * (isize + vsize);
            } else {
                MPI_Isend(base, block_size * vsize, MPI_CHAR, right, MessageType::kAllGather, comm,
                          &requests[0]);
		//if(send_block_index == record_block_index)
		traffic[send_block_index*2*(num_procs-1)+worker_number-1+i] = block_size * vsize;
            }
        } else {
            MPI_Isend(base, block_size * vsize, MPI_CHAR, right, MessageType::kAllGather, comm,
                      &requests[0]);
	...
	for(int i = 0; i < worker_number-1; i ++){
	    ...
	    T *base = buffer + blocks[send_block_index].first;
	    block_size = blocks[send_block_index].second;
	    right = (id+1+i)%worker_number;
	    left = (id-1-i+worker_number)%worker_number;
	    need_check = true;
	    if(need_check){
		int nnz = 0;
		for(int j = 0; j < block_size; ++ j){
		    if(need_send_index[right*block_size+j] == 1)
			nnz ++;
		}
		if(nnz * (isize + vsize) < block_size * vsize){
		    int k = 0;
		    ...
		    memcpy(value_buffer + nnz, index_buffer, isize * nnz);
		    MPI_Isend(value_buffer, nnz * (isize + vsize), MPI_CHAR, right, MessageType::kAllGather, comm, &requests[0]);
		    //if(send_block_index == record_block_index)
		    traffic[send_block_index*2*(num_procs-1)+worker_number-1+i] = nnz * (isize + vsize);
		} else{
		    MPI_Isend(base, block_size * vsize, MPI_CHAR, right, MessageType::kAllGather, comm, &requests[0]);
		    //if(send_block_index == record_block_index)
		    traffic[send_block_index*2*(num_procs-1)+worker_number-1+i] = block_size * vsize;
		}
	    } else {
