...
LRTronOptimizer::LRTronOptimizer(const double *y, double *msgX, int dimension, double rho, int max_iterations,
                                 double epsilon, double cg_epsilon, SparseDataset *dataset, neighbors *neighbors) :
        y_(y), sumX(msgX), dimension_(dimension), rho_(rho),
        max_iterations_(max_iterations), epsilon_(epsilon),
        cg_epsilon_(cg_epsilon), dataset_(dataset), nears(neighbors) {
    ...
    numsofneighbors = nears->neighborsNums - 1;
}
...
//        sum += temp * temp;
        //sum+=y_[i]*x[i]+rho_*(nears->neighborsNums-1)*x[i]*x[i]-rho_*((nears->neighborsNums-2)*x[i]+sumX[i])*x[i];
        //sum+=rho_*numsofneighbors*x[i]*x[i]+(y_[i]-rho_*((numsofneighbors-1)*x[i]+sumX[i]))*x[i];
        //sum+=0.5*rho_*numsofneighbors*x[i]*x[i]+sumX[i]*x[i];
...
void LRTronOptimizer::gradient(const double *x, double *g) {
    for (int i = 0; i < dimension_; ++i) {
        ...
        //g[i]=y_[i]+2*rho_*(nears->neighborsNums-1)*x[i]-rho_*(sumX[i]+(nears->neighborsNums-2)*x[i]));
        //g[i]=2*rho_*numsofneighbors*x[i]+(y_[i]-rho_*((numsofneighbors-1)*x[i]+sumX[i]));
        //g[i]= numsofneighbors*rho_*x[i]+sumX[i];
