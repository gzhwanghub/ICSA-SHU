...
void IntragroupBroadcast(T *buffer, int count, int id, std::vector<int> &member_list, MPI_Comm comm) {
    ...
    if (id != leader_id) {
       ...
       /* int complete = 0;
        ...
        Recv(buffer,count,leader_id,MessageType::kIntragroupBroadcast,comm,MPI_STATUS_IGNORE);
    } else {
        /*std::cout<<"leader broadcast: "<<leader_id<<std::endl;
