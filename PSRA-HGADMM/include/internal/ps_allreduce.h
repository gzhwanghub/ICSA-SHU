...
#include "../../include/internal/reduce_operator.h"
#include "../../include/internal/simple_allreduce.h"
#include "../../include/internal/p2p_communication.h"
...
//稀疏版本的算法和稠密版本的步骤基本一致，只是在发送前判断是否满足稀疏传输条件
    ...
    void SparsePsAllreduce(T *buffer, int count, int id, std::vector<int> &worker_list, MPI_Comm comm) {
        ...
        block_size = blocks[my_index].second;
        for (int i = 0; i < worker_number - 1; ++i) {
            MPI_Recv(recv_buffer, block_size * vsize, MPI_CHAR, MPI_ANY_SOURCE, MessageType::kScatterReduce, comm,
                     &status);
            int nnz = 0;
        ...
        if (need_check) {
            ...
            if (nnz * (isize + vsize) < block_size * vsize) {
                ...
                for (int i = 0; i < worker_number; ++i) {
                    if (i != my_index) {
                        request_list.emplace_back();
                        MPI_Isend(value_buffer, nnz * (isize + vsize), MPI_CHAR, worker_list[i],
                                  MessageType::kAllGather,
