...
//void RingAllreduce(T *buffer, int count, int id, int *worker_list, MPI_Comm comm, int QuantifyPart) {
...
//                  &recv_req);
//            MPI_Send(buffer + blocks[send_block_index].first, blocks[send_block_index].second, MPI_DOUBLE,
//                     right, MessageType::kScatterReduce, comm);
//            MPI_Wait(&recv_req, &recv_status);
            /*只有rcv1可以运行其他高维数据集无法运行*/
