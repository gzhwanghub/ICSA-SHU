...
namespace comlkit {
    ...
    Vector SVRDual(vector <SparseFeature> &features, Vector &y, int solver_type, double lambda, double p, double eps,
                   int max_iter, const int verbosity) {
        int l = features.size();
        double C = 1 / lambda;
        ...
        if (solver_type == L2R_L1LOSS_SVR_DUAL) {
            ...
            lambda = 0.5 * lambda;
            upper_bound = INF;
//		c = new L2SmoothSVRLoss<SparseFeature>(w_size, features, y, lambda, p); // 注释掉了，由于修改
        }
