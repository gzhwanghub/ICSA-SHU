...
Vector gdBarzilaiBorwein(const ContinuousFunctions& c, const Vector& x0, double alpha, const double gamma,
	...
	{
		double gg = g*g;
		xnew = x - alpha*g; // Updating x based on gradient method.
		c.eval(xnew, fnew, gnew); // Updating f and g.
		funcEval++;
		// double fgoal = f - gamma*alpha*gg; // Armijo
		// Backtracking line search // Backtracking line search for step size.
