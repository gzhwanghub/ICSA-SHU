...
L1LogisticLoss<Feature>::L1LogisticLoss(int m, std::vector<Feature>& features, Vector& y, Vector& new_y, Vector& new_z, double lambda, double rho) :
	ContinuousFunctions(true, m, features.size()), features(features), y(y), new_y_(new_y), new_z_(new_z), lambda(lambda), rho_(rho)
{
	if (n > 0)
