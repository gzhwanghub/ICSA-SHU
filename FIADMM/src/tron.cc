	...
	*reach_boundary = false;
	...
	while (1)
	{
		if (norm(r) <= cgtol)
			break;
		cg_iter++;
		c.evalHessianVectorProduct(x, d, Hd); // 求梯度方向：\nabla^2 f(w)s = s + C*X^T (D(Xs))

		alpha = rTr/(d*Hd); // CG求步长
		s += alpha*d;
		if (norm(s) > delta)
		{
...
Vector tron(const ContinuousFunctions& c, int myid, const Vector& x0, const int maxEval, const double TOL, int verbosity){
	double eta0 = 1e-4, eta1 = 0.25, eta2 = 0.75;
	// Parameters for updating the trust region size delta.
	double sigma1 = 0.25, sigma2 = 0.5, sigma3 = 4;
	int m = c.size(); // 初始化没问题，分组那里出问题了。
	int i, cg_iter;
	double delta, snorm;
