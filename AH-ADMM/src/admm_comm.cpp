...
AdmmComm::AdmmComm(int proc_id, int proc_size, int master_id, std::vector<int> worker_id, int partial_barrier,int bounded_delay, int master_to_worker_data_size, int worker_to_master_data_size)
        ...
        master_to_worker_data_size_(master_to_worker_data_size),
        worker_to_master_data_size_(worker_to_master_data_size){
...
void AdmmComm::RunMaster(){
    int k = 0;
    double comm_start=0.0, comm_end=0.0, send_start=0.0;
    bool is_stop = false;
    //1. 初始化循环非阻塞接收
    ...
    while (true){
        ...
        comm_time_m += (comm_end - comm_start);
        for (int i = 0; i < worker_nums_; ++i){
            ++t_worker[i];
        }
        ...
        cal_time_m += (double)(send_start-comm_end);
        k++;
    }
}
